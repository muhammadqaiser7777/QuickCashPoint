...
export class Loans implements OnInit {
  questions = [
    ...
    { label: 'What is your State?', type: 'text', inputType: 'text', placeholder: 'e.g., CA' },
    ...
    { label: 'Date of Birth', type: 'dob' }, // <-- DOB step
{ 
  label: 'Phone Number (10 digits without +1)', 
  type: 'text', 
  inputType: 'tel', 
  placeholder: 'e.g. 1234567890',
  maxLength: 10 // enough for formatting (###) ###-####
},
    { label: 'Email Address', type: 'email', placeholder: 'e.g. example@gmail.com' },
    { label: 'Last 4 Digits of SSN', type: 'password', placeholder: 'e.g. 1234', maxLength: 4 },
    {
    ...
    {label: 'Pay Frequency', type: 'select', options: [
      'Weekly',
      'Bi-Weekly',
      'Monthly',
      'Semi-Monthly',
    ...
    { label: 'Unsecured Debt', type:'select', options:[
      'Less than 10K',
      '15K',
      '20K',
      '20K +',
    ]},
    {label: 'Active Military', type:'select', options:[
      'YES',
      'NO',
    ]},
    {
  ...
  public readonly ZIP_CODE_QUESTION_INDEX = 1;
  private readonly CITY_QUESTION_INDEX = 2;
  private readonly STATE_QUESTION_INDEX = 3;
  private readonly DOB_QUESTION_INDEX = 8;
  private readonly PHONE_QUESTION_INDEX = 9;
  private readonly EMAIL_QUESTION_INDEX = 10;
  private readonly SSN_QUESTION_INDEX = 11;
  ...
  public readonly tcpaConsent: string = 'YES';
  ...
  async submitForm() {
    ...
    const emptyIndex = this.answers.findIndex(a => !a || a === '');
    if (emptyIndex !== -1) {
    ...
    try {
      ...
      // Normalize Date of Birth to mm-dd-yyyy in payload
      ...
      const payload = {
        // Canonical fields expected by backend
        ...
        ...(sessionStorage.getItem('aff_id') || this.affId ? { aff_id: sessionStorage.getItem('aff_id') || this.affId } : {}),
        ...(sessionStorage.getItem('sub_aff_id') || this.subAffId ? { sub_aff_id: sessionStorage.getItem('sub_aff_id') || this.subAffId } : {}),
        ...(sessionStorage.getItem('tid') || this.tid ? { tid: sessionStorage.getItem('tid') || this.tid } : {}),
        ...(sessionStorage.getItem('universalLeadid') || this.universalLeadid ? { universalLeadid: sessionStorage.getItem('universalLeadid') || this.universalLeadid } : {}),
        ...(sessionStorage.getItem('xxTrustedFormCertUrl') || this.trustedFormCertUrl ? { xxTrustedFormCertUrl: sessionStorage.getItem('xxTrustedFormCertUrl') || this.trustedFormCertUrl } : {}),
      ...
      // Stop TrustedForm polling and remove script since we're not live yet
      try {
